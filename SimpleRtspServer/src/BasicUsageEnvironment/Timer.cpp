...
TimerManager::TimerManager(EventScheduler* scheduler) : mPoller(scheduler->poller()), mLastTimerId(0) 
{
#ifndef WIN32
...
#else
    scheduler->setTimerManagerReadCallback(readCallback, this);//设置定时任务回调
#endif // !WIN32
}
        ...
        if (timestamp > timer.mTimestamp || expire == 0) {
            ...
            if (timer.mRepeat) {
                if (timerEventIsStop) {
                    ...
                    mEvents.insert(std::make_pair(timer.mTimestamp, timer));//重新插入触发该超时事件
                }
            } else {
                mTimers.erase(timer.mTimerId);
            }
